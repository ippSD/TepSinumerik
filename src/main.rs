...
// Drill empty square.
fn do_square(mut file: &File, square: Square, lc: &mut u32) -> std::io::Result<()>{
...
// Drill empty circle.
fn do_circle(mut file: &File, circle: Circle, lc: &mut u32) -> std::io::Result<()>{
...
fn setup(mut file: &File, lc: &mut u32) -> std::io::Result<()>{
    ...
    lc.add_assign(10);
    operations = operations + format!("N{0} S2000 M03\r\n", lc).as_str();
